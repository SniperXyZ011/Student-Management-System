...
func main() {
	...
	router.HandleFunc("GET /", func(w http.ResponseWriter, r *http.Request) {
		...
		data := []byte ("Welcome to my first go server")
		w.Write(data)
	})
	...
	slog.Info("Server has started, %s", slog.String("Address",cfg.Addr)) //we are using slog, which is nothing but structured log, kafi accha hai for production logs ke liye, ex : 2025/12/17 23:20:56 INFO Server has started, %s Address=localhost:8080
